...
void set_EEPROM_Default() {
    ...
    eeprom_config.ip[0]=10;
    eeprom_config.ip[1]=0;
    eeprom_config.ip[2]=0;
    eeprom_config.ip[3]=99;
  
    // set the default GATEWAY. In this case its 192.168.0.254
    eeprom_config.gateway[0]=10;
    eeprom_config.gateway[1]=0;
    eeprom_config.gateway[2]=0;
    eeprom_config.gateway[3]=200;
    ...
    eeprom_config.dns_server[0]=8;
    eeprom_config.dns_server[1]=8;
    eeprom_config.dns_server[2]=8;
    eeprom_config.dns_server[3]=8;

    // set the default Webserver Port. In this case its Port 80
    eeprom_config.webserverPort=80;
    
    eeprom_config.dustPin=0;
...
void print_EEPROM_Settings() {
    ...
    Serial.print(" dustPin ");
    Serial.print(eeprom_config.dustPin);
    Serial.println("");
    
    Serial.print(" dustLedPin ");
    Serial.print(eeprom_config.dustLedPin);
    Serial.println("");
    
    Serial.print(" dustMultiplier ");
    Serial.print(eeprom_config.dustMultiplier);
    Serial.println("");
    
    Serial.print("forwardIP: ");
    for(int i = 0; i<4; i++) {
      Serial.print(eeprom_config.forwardIp[i]);
      if (i<3) {
        Serial.print('.');
      }
    }
    ...
    Serial.print(" forwardPort ");
    Serial.print(eeprom_config.forwardPort);
    Serial.println("");
...
void setupDustSensor() {
  ...
  pinMode(eeprom_config.dustLedPin, OUTPUT);
...
*/
P(Page_start) = "<html><head><title>Particle Sensor</title></head><body>\n";
...
P(Form_input_size5) = "\" maxlength=\"5\" size=\"5";
P(Form_input_end) = "\">\n";
...
P(DUST_PIN) = "Dust pin: ";
P(DUST_LED_PIN) = "Dust LED pin: ";
P(DUST_MULTIPLIER) = "Dust multiplier: ";
P(FORWARD_ADDRESS) = "Forward Address: ";
P(FORWARD_PORT) = "Forward Port: ";
...
void setupNetHTML(WebServer &server, WebServer::ConnectionType type, char *url_tail, bool tail_complete)
{
  ...
  if (strlen(url_tail)) {
    while (strlen(url_tail)) {
      ...
      if (rc != URLPARAM_EOS) {
        ...
        // dustPin
        if (param_number == 25) {
          eeprom_config.dustPin=atoi(value);
        } 
        
        // dustLedPin
        if (param_number == 26) {
          eeprom_config.dustLedPin=atoi(value);
        }
        
        // dustMultiplier
        if (param_number == 27) {
          eeprom_config.dustMultiplier=atof(value);
        }
        
        // read forward address
        if (param_number >=28 && param_number <=31) {
          eeprom_config.forwardIp[param_number-28]=atoi(value);
        }
        
        
        // read forward address
        if (param_number ==32) {
          eeprom_config.forwardPort=atof(value);
        }
  ...
  server.printP(table_tr_start);
  server.printP(table_td_start);
  server.printP(DUST_PIN);
  server.printP(table_td_end);
  server.printP(table_td_start);
  server.printP(Form_input_text_start);
  server.print(25);
  server.printP(Form_input_value);
  server.print(eeprom_config.dustPin);
  server.printP(Form_input_size3);
  server.printP(Form_input_end);
  server.printP(table_td_end);
  server.printP(table_tr_end);
  ...
  server.printP(table_tr_start);
  server.printP(table_td_start);
  server.printP(DUST_LED_PIN);
  server.printP(table_td_end);
  server.printP(table_td_start);
  server.printP(Form_input_text_start);
  server.print(26);
  server.printP(Form_input_value);
  server.print(eeprom_config.dustLedPin);
  server.printP(Form_input_size3);
  server.printP(Form_input_end);
  server.printP(table_td_end);
  server.printP(table_tr_end);
  ...
  for (int a=0;a<4;a++) {
    ...
    server.print(a+28);
    server.printP(Form_input_value);
    server.print(eeprom_config.forwardIp[a]);
    server.printP(Form_input_size3);
    server.printP(Form_input_end);
  }
  server.printP(table_td_end);
...
* This function is used to send the index.html to the client.
*/
void sensorData(WebServer &server, WebServer::ConnectionType type, char *url_tail, bool tail_complete)
{
  /* this line sends the standard "we're all OK" headers back to the
     browser */
...
void setup()
{
  ...
  webserver->addCommand("setupNet.html", &setupNetHTML);
   ...
   webserver->addCommand("sensor", &sensorData);
...
int measureDust() {
  ...
  float dustDensity=0.0;
   ...
   digitalWrite(eeprom_config.dustLedPin, LOW); // power on the LED
   delayMicroseconds(samplingTime);
   
   voMeasured = analogRead(eeprom_config.dustPin); // read the dust value
   
   delayMicroseconds(deltaTime);
   digitalWrite(eeprom_config.dustLedPin, HIGH); // turn the LED off
   delayMicroseconds(sleepTime);
   
   calcVoltage = voMeasured * (5.0 / 1024);
   dustDensity = (eeprom_config.dustMultiplier * calcVoltage) * 1000;
